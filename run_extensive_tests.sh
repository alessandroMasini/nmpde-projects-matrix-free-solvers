                                    ...
                                    for tol in $TOL; do
                                        for ((i=0; i<N_TESTS; i++)); do
                                            ...
                                            if [[ "$solver" == "mf" ]]; then
                                                ...
                                                case "$simd" in
                                                    0)  run_solver_attempt "run $i" "$test_base_dir" "$solver" "$simd" "$problem" "$fe_deg" "$n_additional_refinements" "$n_ranks" "$n_threads" "$delta_t" "$max_iters" "$tol" mpirun --report-bindings -n "$n_ranks" --bind-to core --map-by slot:PE="$n_threads" ./matrix_free_no_simd "$n_threads" "$simd" "$problem" "$fe_deg" "$n_additional_refinements" "$delta_t" "$max_iters" "$tol";;
                                                    1)  run_solver_attempt "run $i" "$test_base_dir" "$solver" "$simd" "$problem" "$fe_deg" "$n_additional_refinements" "$n_ranks" "$n_threads" "$delta_t" "$max_iters" "$tol" mpirun --report-bindings -n "$n_ranks" --bind-to core --map-by slot:PE="$n_threads" ./matrix_free_simd "$n_threads" "$simd" "$problem" "$fe_deg" "$n_additional_refinements" "$delta_t" "$max_iters" "$tol";;
                                                ...
                                                case "$simd" in
                                                    0)
                                                        ...
                                                        run_solver_attempt "run $i" "$test_base_dir" "$solver" "0" "$problem" "$fe_deg" "$n_additional_refinements" "$n_ranks" "$n_threads" "$delta_t" "$max_iters" "$tol" mpirun --report-bindings -n "$n_ranks" --bind-to core --map-by slot:PE="$n_threads" ./matrix_based "$n_threads" "$problem" "$fe_deg" "$n_additional_refinements" "$delta_t" "$max_iters" "$tol";;
                                                    1)  continue;;
                                                    *) echo "Unknown simd value: $simd";;
                                                esac
                                            fi
