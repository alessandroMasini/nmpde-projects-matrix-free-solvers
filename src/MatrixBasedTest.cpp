...
public:
    ...
    dealii::VectorizedArray<float> value(const dealii::Point<dim, dealii::VectorizedArray<float>> &p, const unsigned int component = 0) const override
    {
        return do_compute_value<dealii::VectorizedArray<float>>(p, component);
    }

    dealii::VectorizedArray<double> value(const dealii::Point<dim, dealii::VectorizedArray<double>> &p, const unsigned int component = 0) const override
    {
    ...
    {
        return 2 * 1.0e-2 * (M_PI * M_PI) * std::sin(M_PI * p[0]) * std::sin(M_PI * p[1]) + M_PI * std::cos(M_PI * p[0]) * std::sin(M_PI * p[1]) + M_PI * std::cos(M_PI * p[1]) * std::sin(M_PI * p[0]) + std::sin(M_PI * p[0]) * std::sin(M_PI * p[1]);
    }
};
...
int main(int argc, char *argv[])
{
    ...
    DirichletBoundaries<2> db;
    NeumannBoundaries<2> nb;

    db[0] = std::make_shared<ADR::ConstantRealFunction<2>>(0.0);
    nb[1] = std::make_shared<ADR::ConstantRealFunction<2>>(0.0);

    // Currently not used
    data.dirichlet_boundaries = db;
    data.neumann_boundaries = nb;
